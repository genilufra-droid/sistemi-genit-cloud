...
p = Path('apps/api/src/phase4-traceability.js')
s = p.read_text()

schema_anchor = "  const qualitySchema = z.object({"
schema_block = """  const weightDraftSchema = z.object({
...
"""
if schema_block not in s:
    if schema_anchor not in s:
        raise SystemExit('Quality schema anchor missing')
    s = s.replace(schema_anchor, schema_block + schema_anchor, 1)

fn_anchor = "async function nextReceiptNo(client, tenantId, companyId, documentDate) {"
fn_block = """async function nextWeightNo(client, tenantId, companyId, documentDate) {
...
"""
if fn_block not in s:
    if fn_anchor not in s:
        raise SystemExit('Receipt number anchor missing')
    s = s.replace(fn_anchor, fn_block + fn_anchor, 1)

route_anchor = "  app.post('/api/weights/:id/post-receipt', authRequired, requireRoles(...WRITE_ROLES), async (req,res,next) => {"
route_block = """  async function validateWeightRelations(client, user, input) {
...
if route_block not in s:
    if route_anchor not in s:
        raise SystemExit('Post receipt route anchor missing')
