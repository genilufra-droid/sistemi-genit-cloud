...
const readyNew='  App.SGPhase42={load:loadData}; global.SGPhase42=App.SGPhase42;';
if(js.includes(readyOld))js=js.replace(readyOld,readyNew);
else if(!js.includes(readyNew))throw new Error('Mungon flamuri i gatishmërisë Phase 4.2.');
...
['Proces & Paketim Cloud','editProcessOrderOnline','editPackagingOrderOnline','exportProcessOrderOnlinePDF','exportPackagingOrderOnlineExcel','global.SGPhase42=App.SGPhase42'].forEach(function(marker){if(!check.includes(marker))throw new Error('Mungon '+marker+' në HTML.');});
