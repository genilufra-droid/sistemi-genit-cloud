...
if (!check.includes('this._p4EditingWeight = wf || null') || !check.includes("|| editing.supplierId || ''") || !check.includes("|| editing.productId || ''") || !check.includes("|| editing.warehouseId || ''") || !check.includes('farmSelect.value = wf.farmId') || !check.includes('parcelSelect.value = wf.parcelId')) throw new Error('Fallback-u i Draftit ose vlerat e Fermës/Parcelës nuk u injektuan.');
