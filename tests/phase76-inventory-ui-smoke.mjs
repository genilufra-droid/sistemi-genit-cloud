...
for(const report of ['stock-current','stock-by-warehouse','stock-by-location','stock-by-product','stock-by-lot','free-reserved','product-ledger','lot-ledger','receipts-register','deliveries-register','moves-history','internal-transfers','stock-at-date','valuation','discrepancies','slow-stock','below-minimum','turnover','in-out-period','supplier-lot-customer'])assert.ok(ui.includes(report)||report==='stock-current',`Mungon raporti UI: ${report}`);
...
console.log('PHASE76_UI_SUCCESS document-template=photo-style reports=20 no-dropdown no-observer');
