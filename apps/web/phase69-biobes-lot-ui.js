...
(function(global){
  ...
  Cloud.request=async function(path,options){
    ...
    if((method==='POST'||method==='PATCH')&&/^\/api\/trace\/weights(?:\/[^/]+)?$/.test(path)){
      ...
      var period=value('wf-biobes-period')||'I';
      if(weightId&&['I','II','III'].indexOf(period)>=0){
        await baseRequest('/api/trace/weights/'+encodeURIComponent(weightId)+'/biobes-meta',{method:'PATCH',body:{harvestPeriod:period}});
        if(result&&typeof result==='object')result.harvest_period=period;
      }
    }
  ...
  function injectWeightFields(){
    ...
    var details=(App.data&&App.data.weightForms||[]).find(function(row){return row.id===App.__sg64CurrentWeightId;})||{};
    var period=String(details.harvestPeriod||details.harvest_period||'I').toUpperCase();
    ...
    group.querySelector('select').value=['I','II','III'].indexOf(period)>=0?period:'I';
  ...
  if(typeof baseWeightView==='function')App._viewWeightForm=async function(){var result=await baseWeightView.apply(this,arguments);injectWeightFields();return result;};
