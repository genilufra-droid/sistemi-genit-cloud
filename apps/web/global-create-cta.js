...
(function (global) {
  ...
  function prefillNative(nameIds, codeIds, query, extra) {
    if (!query && !extra) return;
    function apply() {
      var name = inputByIds(nameIds || []);
      var code = inputByIds(codeIds || []);
      if (name && query && !name.value) { name.value = query; name.dispatchEvent(new Event('input', { bubbles: true })); }
      if (code && query && !code.value) { code.value = normalizeCode(query); code.dispatchEvent(new Event('input', { bubbles: true })); }
      if (typeof extra === 'function') extra();
      if (name) name.focus(); else if (code) code.focus();
      return Boolean(name || code);
    }
    if (apply()) return;
    setTimeout(apply, 80); setTimeout(apply, 260); setTimeout(apply, 600);
  }
  function canCreate(definition) {
    if (!definition) return false;
    if (definition.serverType && Object.prototype.hasOwnProperty.call(serverCapabilities, definition.serverType)) return serverCapabilities[definition.serverType] === true;
  ...
  if (baseCloseModal) App.closeModal = function () { if (quickCreateContext) return this.cancelQuickCreate(); return baseCloseModal(); };

  var entityDefinitions = [];
  function register(definition) { entityDefinitions = entityDefinitions.filter(function (item) { return item.key !== definition.key; }); entityDefinitions.unshift(definition); return definition; }
  function nativeDefinition(input) {
    return {
      ...
      create: function (query) {
        ...
        prefillNative(input.nameIds || [], input.codeIds || [], query, input.extra);
      }
    };
  }
  function genericDefinition(input) {
    return { key:input.key, match:input.match, label:input.label, icon:input.icon, permission:input.permission||'masters.manage', serverType:input.serverType, create:function(query){App.openGenericMasterQuickCreate(input.key,input.serverType,input.label,query);} };
  }
  ...
  register(nativeDefinition({ key:'cashAccount', match:/ark[eë]|cash account|llogari arke/, label:'Arkë', icon:'💵', serverType:'CASH_ACCOUNT', open:['editFinanceAccount'], nameIds:['sg5-a-name'], codeIds:['sg5-a-code'], extra:function(){var kind=document.getElementById('sg5-a-kind');if(kind){kind.value='CASH';kind.dispatchEvent(new Event('change',{bubbles:true}));}} }));
  register(nativeDefinition({ key:'bankAccount', match:/bank[eë]|iban|bank account|llogari banke/, label:'Llogari Bankare', icon:'🏦', serverType:'BANK_ACCOUNT', open:['editFinanceAccount'], nameIds:['sg5-a-name'], codeIds:['sg5-a-code'], extra:function(){var kind=document.getElementById('sg5-a-kind');if(kind){kind.value='BANK';kind.dispatchEvent(new Event('change',{bubbles:true}));}} }));
  ...
  function definitionByKey(key) { for (var i=0;i<entityDefinitions.length;i+=1) if(entityDefinitions[i].key===key)return entityDefinitions[i]; return null; }
  ...
  App.openGenericMasterQuickCreate = function (key, serverType, label, query) {
    ...
    var body = '<div class="sg-global-master-form"><div class="form-group"><label>Kodi</label><input id="sg-gm-code" value="'+attr(normalizeCode(query))+'" maxlength="100"></div><div class="form-group"><label>Emri *</label><input id="sg-gm-name" value="'+attr(query||'')+'" maxlength="220" required autofocus></div><div class="form-group sg-gm-wide"><label>Përshkrimi</label><textarea id="sg-gm-description" maxlength="3000"></textarea></div><div class="sg-gm-note sg-gm-wide">Rekordi ruhet në PostgreSQL dhe zgjidhet automatikisht në dokumentin aktual.</div></div>';
    var footer = '<button class="btn btn-outline" onclick="App.cancelQuickCreate()">Anulo</button><button id="sg-gm-save" class="btn btn-primary" onclick="App.saveGenericMasterQuickCreate(\''+attr(key)+'\',\''+attr(serverType)+'\')">Ruaj dhe zgjidh</button>';
  ...
  App.saveGenericMasterQuickCreate = async function (key, serverType) {
    ...
    var name=(document.getElementById('sg-gm-name')||{}).value||'',code=(document.getElementById('sg-gm-code')||{}).value||'',description=(document.getElementById('sg-gm-description')||{}).value||'';
    ...
    try{var row=await Cloud.request('/api/master-data/'+encodeURIComponent(serverType),{method:'POST',body:{companyId:companyId(),code:code.trim(),name:name.trim(),description:description.trim(),metadata:{source:'GLOBAL_SEARCH_CREATE',sourceView:quickCreateContext&&quickCreateContext.sourceView||''},active:true}});this.completeQuickCreate(key,row);}catch(error){this.toast(error.message||String(error),'error');if(button)button.disabled=false;}
  };

  var baseSaveFinanceAccount = App.saveFinanceAccount;
  if (typeof baseSaveFinanceAccount === 'function') {
    App.saveFinanceAccount = async function (id) {
      var isCashQuick = !id && this.hasQuickCreateContext('cashAccount');
      var isBankQuick = !id && this.hasQuickCreateContext('bankAccount');
      if (!isCashQuick && !isBankQuick) return baseSaveFinanceAccount.apply(this, arguments);
      try {
        Auth.requirePermission('masters.manage');
        var body={companyId:companyId(),accountKind:isBankQuick?'BANK':'CASH',code:(document.getElementById('sg5-a-code')||{}).value||'',name:(document.getElementById('sg5-a-name')||{}).value||'',currency:(document.getElementById('sg5-a-currency')||{}).value||'ALL',openingBalance:num((document.getElementById('sg5-a-opening')||{}).value),openingDate:(document.getElementById('sg5-a-date')||{}).value||today(),bankName:(document.getElementById('sg5-a-bank')||{}).value||'',iban:(document.getElementById('sg5-a-iban')||{}).value||'',accountNumber:(document.getElementById('sg5-a-number')||{}).value||'',active:((document.getElementById('sg5-a-active')||{}).value||'true')==='true',notes:(document.getElementById('sg5-a-notes')||{}).value||''};
        var row=await Cloud.request('/api/finance/accounts',{method:'POST',body:body});
        if(Cloud.loadFinance)await Cloud.loadFinance(false);
        this.completeQuickCreate(isBankQuick?'bankAccount':'cashAccount',row);
        return row;
  ...
  if (SAC && typeof SAC._render === 'function' && !SAC.__sgCreatePatched) {
    ...
    SAC._render=function(resultsEl,items,opts,instId){
      baseRender(resultsEl,items,opts,instId);if(items&&items.length)return;
      var instance=SAC.instances&&SAC.instances[instId],input=instance&&instance.input,definition=(opts&&opts.createDefinition)||definitionForInput(input);
      if(!definition||!canCreate(definition))return;
      var empty=resultsEl.querySelector('.sac-empty');if(!empty){empty=document.createElement('div');empty.className='sac-empty';empty.textContent='Nuk u gjet asnjë rezultat';resultsEl.appendChild(empty);}else if(lower(empty.textContent).indexOf('nuk u gjet')<0)empty.insertAdjacentHTML('afterbegin','<div>Nuk u gjet asnjë rezultat</div>');
      if(empty.querySelector('.sg-create-no-result'))return;
      var query=input?input.value.trim():'',button=document.createElement('button');button.type='button';button.className='sg-create-no-result';button.innerHTML='<span>'+String(definition.icon||'＋')+'</span><strong>+ Shto të ri</strong><em>'+esc(definition.label||'')+'</em>'+(query?'<small>“'+esc(query)+'”</small>':'');button.addEventListener('mousedown',function(event){event.preventDefault();event.stopPropagation();});button.addEventListener('click',function(event){event.preventDefault();event.stopPropagation();resultsEl.classList.remove('show');App.createFromNoResult(input,definition);});empty.appendChild(button);
  ...
  function viewActions(view){var map={products:[buttonAction('+ Shto Artikull',function(){App.editProduct();},'📦','masters.manage')],partners:[buttonAction('+ Shto Furnitor / Fermer',function(){App.editPartner('supplier');},'🌱','masters.manage'),buttonAction('+ Shto Klient',function(){App.editPartner('customer');},'👤','masters.manage')],weightList:[buttonAction('+ Shto Peshim / Pranim',function(){App.navigate('weightForm');},'⚖️')],traceLots:[buttonAction('+ Shto Peshim / Pranim',function(){App.navigate('weightForm');},'⚖️')],traceProcesses:[buttonAction('+ Shto Urdhër Pune',function(){invoke(['editProcessOrderOnline','editProcessBatch']);},'⚙️'),buttonAction('+ Shto Paketim',function(){invoke(['editPackagingOrderOnline']);},'📦')],salesList:[buttonAction('+ Shto Shitje',function(){App.navigate('salesForm');},'🧾')],expenses:[buttonAction('+ Shto Shpenzim',function(){invoke(['editExpense']);},'💸')],logisticsVehicles:[buttonAction('+ Shto Automjet',function(){invoke(['editLogisticsVehicle','editVehicle']);},'🚚','masters.manage')],exportShipments:[buttonAction('+ Shto Ngarkesë',function(){invoke(['editExportShipment','editShipment']);},'🌍')],fixedAssets:[buttonAction('+ Shto Aset / Makineri',function(){invoke(['editFixedAsset','editAsset']);},'🏭','masters.manage')],financeAccounts:[buttonAction('+ Shto Llogari',function(){App.editFinanceAccount();},'🏦','masters.manage')]};return(map[view]||[]).filter(function(item){return!Auth||!Auth.hasPermission||Auth.hasPermission(item.permission);});}
  function existingAddButtons(scope){return Array.prototype.slice.call(scope.querySelectorAll('button,a.btn')).filter(function(element){if(element.closest('.sg-empty-create-actions'))return false;var label=lower(element.textContent);return/^\s*\+/.test(element.textContent||'')||/\bshto\b|\bi ri\b|\be re\b/.test(label);}).slice(0,3).map(function(source){return buttonAction((source.textContent||'+ Shto').trim(),function(){source.click();},'＋');});}
  function enhanceEmptyStates(){var content=document.getElementById('content');if(!content)return;Array.prototype.forEach.call(content.querySelectorAll('.empty-report'),function(empty){if(empty.dataset.sgCreateEnhanced==='1')return;empty.dataset.sgCreateEnhanced='1';if(lower(empty.textContent).indexOf('nuk u gjet')<0&&lower(empty.textContent).indexOf('nuk ka')<0)empty.insertAdjacentHTML('afterbegin','<strong>Nuk u gjet asnjë rezultat</strong>');var scope=empty.closest('.card')||content,actions=existingAddButtons(scope);if(!actions.length)actions=viewActions(App.currentView);if(!actions.length)return;var holder=document.createElement('div');holder.className='sg-empty-create-actions no-print';actions.forEach(function(item){var button=document.createElement('button');button.type='button';button.className='btn btn-primary';button.innerHTML='<span>'+item.icon+'</span> '+esc(item.label);button.addEventListener('click',item.action);holder.appendChild(button);});empty.appendChild(holder);});}

  async function loadCapabilities(){if(!Cloud||!Cloud.request)return;try{var rows=await Cloud.request('/api/master-data/capabilities');(rows||[]).forEach(function(row){serverCapabilities[row.entityType]=Boolean(row.canCreate);});}catch(_){}}
  var observer=new MutationObserver(function(){enhanceEmptyStates();});
  function start(){var content=document.getElementById('content');if(content)observer.observe(content,{childList:true,subtree:true});enhanceEmptyStates();loadCapabilities();}
