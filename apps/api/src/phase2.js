...
export function installPhase2Routes({ app, pool, authRequired, requireRoles, assertCompanyAccess, audit, emitTenant }) {
  ...
  app.delete('/api/products/:id', authRequired, requireRoles(...WRITE_ROLES), async (req,res,next)=>{
    const client=await pool.connect();
    try {
      await client.query('BEGIN');
      const current=await getScoped('products',req.params.id,req.user,client);
      await client.query('DELETE FROM products WHERE id=$1 AND tenant_id=$2',[current.id,req.user.tenant_id]);
      await audit({tenantId:req.user.tenant_id,userId:req.user.id,action:'PRODUCT_DELETE',entityType:'product',entityId:current.id,companyId:current.company_id,metadata:{name:current.name,softDelete:false},ip:req.ip},client);
      await client.query('COMMIT');
      emitTenant(req.user.tenant_id,'products',{action:'deleted',id:current.id});
      res.json({ok:true,id:current.id});
    } catch(e) {
      await client.query('ROLLBACK');
      if(e?.code==='23503'){
        e.status=409;
        e.message='Artikulli është përdorur në dokumente ose në stok. Përdor Arkivo.';
      }
      next(e);
    } finally { client.release(); }
  });
  ...
  app.delete('/api/partners/:id', authRequired, requireRoles(...WRITE_ROLES), async (req,res,next)=>{
    const client=await pool.connect();
    try {
      await client.query('BEGIN');
      const current=await getScoped('business_partners',req.params.id,req.user,client);
      await client.query('DELETE FROM business_partners WHERE id=$1 AND tenant_id=$2',[current.id,req.user.tenant_id]);
      await audit({tenantId:req.user.tenant_id,userId:req.user.id,action:'PARTNER_DELETE',entityType:'business_partner',entityId:current.id,companyId:current.company_id,metadata:{name:current.name,softDelete:false},ip:req.ip},client);
      await client.query('COMMIT');
      emitTenant(req.user.tenant_id,'partners',{action:'deleted',id:current.id});
      res.json({ok:true,id:current.id});
    } catch(e) {
      await client.query('ROLLBACK');
      if(e?.code==='23503'){
        e.status=409;
        e.message='Partneri është përdorur në dokumente. Përdor Arkivo.';
      }
      next(e);
    } finally { client.release(); }
  });
