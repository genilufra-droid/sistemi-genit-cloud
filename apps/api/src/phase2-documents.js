...
export function installPhase2DocumentRoutes({ app, pool, authRequired, requireRoles, assertCompanyAccess, audit, emitTenant }) {
  ...
  app.get('/api/documents/:id/trace', authRequired, async (req,res,next) => {
    try {
      const currentResult=await pool.query('SELECT * FROM business_documents WHERE id=$1 AND tenant_id=$2 LIMIT 1',[req.params.id,req.user.tenant_id]);
      const current=currentResult.rows[0];
      if(!current) throw requestError('Dokumenti nuk u gjet.',404);
      await assertCompanyAccess(req.user,current.company_id);
      const documentResult=await pool.query(`
        WITH RECURSIVE ancestors AS (
          SELECT d.* FROM business_documents d WHERE d.id=$1 AND d.tenant_id=$2
          UNION ALL SELECT parent.* FROM business_documents parent JOIN ancestors child ON child.source_document_id=parent.id WHERE parent.tenant_id=$2
        ), descendants AS (
          SELECT d.* FROM business_documents d WHERE d.id=$1 AND d.tenant_id=$2
          UNION ALL SELECT child.* FROM business_documents child JOIN descendants parent ON child.source_document_id=parent.id WHERE child.tenant_id=$2
        )
        SELECT DISTINCT id,doc_type,document_no,document_date,status,created_at,trace_dossier_id
        FROM (SELECT * FROM ancestors UNION ALL SELECT * FROM descendants) chain ORDER BY created_at`,[current.id,req.user.tenant_id]);
      const dossierIds=[...new Set(documentResult.rows.map((row)=>row.trace_dossier_id).filter(Boolean))];
      const documentIds=documentResult.rows.map((row)=>row.id);
      const [weights,finance]=await Promise.all([
        pool.query(`SELECT wt.id,wt.document_no,wt.document_date,wt.status,wt.created_at FROM weight_tickets wt
          WHERE wt.tenant_id=$1 AND (wt.receipt_document_id=ANY($2::uuid[]) OR (cardinality($3::uuid[])>0 AND wt.trace_dossier_id=ANY($3::uuid[])))
          ORDER BY wt.created_at`,[req.user.tenant_id,documentIds,dossierIds]),
        pool.query(`SELECT DISTINCT fd.id,fd.document_type,fd.document_no,fd.document_date,fd.status,fd.created_at,
            fa.account_kind,fa.name AS account_name,pa.business_document_id
          FROM finance_documents fd JOIN finance_accounts fa ON fa.id=fd.account_id
          JOIN payment_allocations pa ON pa.finance_document_id=fd.id
          WHERE fd.tenant_id=$1 AND pa.business_document_id=ANY($2::uuid[]) ORDER BY fd.created_at`,[req.user.tenant_id,documentIds]),
      ]);
      const nodes=[
        ...weights.rows.map((row)=>({kind:'weight_ticket',id:row.id,documentNo:row.document_no,documentDate:row.document_date,status:row.status,label:'WEIGHT_TICKET',createdAt:row.created_at})),
        ...documentResult.rows.map((row)=>({kind:'business_document',id:row.id,documentNo:row.document_no,documentDate:row.document_date,status:row.status,label:row.doc_type,createdAt:row.created_at,current:row.id===current.id})),
        ...finance.rows.map((row)=>({kind:'finance_document',id:row.id,documentNo:row.document_no,documentDate:row.document_date,status:row.status,label:row.document_type,accountKind:row.account_kind,accountName:row.account_name,sourceDocumentId:row.business_document_id,createdAt:row.created_at})),
      ].sort((a,b)=>new Date(a.createdAt||a.documentDate||0)-new Date(b.createdAt||b.documentDate||0));
      res.json({rootDocumentId:current.id,nodes});
    } catch(error) { next(error); }
  });
...
function documentSelectSql() {
  return `SELECT d.*,c.name AS company_name,c.nipt AS company_nipt,c.address AS company_address,
    w.name AS warehouse_name,bp.name AS partner_name,bp.nipt AS partner_nipt,bp.address AS partner_address,
    source.document_no AS source_document_no,
    source.doc_type AS source_document_resolved_type,
    COALESCE((SELECT json_agg(json_build_object(
      'id',linked.id,'documentNo',linked.document_no,'docType',linked.doc_type,
      'status',linked.status,'documentDate',linked.document_date
    ) ORDER BY linked.created_at)
    FROM business_documents linked
    WHERE linked.tenant_id=d.tenant_id AND linked.source_document_id=d.id),'[]'::json) AS linked_documents,
    COALESCE((SELECT json_agg(json_build_object(
    ...
    LEFT JOIN business_documents source ON source.id=d.source_document_id AND source.tenant_id=d.tenant_id`;
}
