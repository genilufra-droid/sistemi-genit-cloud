...
const label = (type) => ({
  PURCHASE_RFQ: 'KËRKESË PËR OFERTË', PURCHASE_ORDER: 'POROSI BLERJE', PURCHASE_RECEIPT: 'FLETË HYRJE', PURCHASE_INVOICE: 'FATURË BLERJE', SUPPLIER_RETURN: 'KTHIM FURNITORI', PURCHASE_RETURN: 'KTHIM TE FURNITORI',
  SALES_QUOTE: 'OFERTË SHITJE', SALES_ORDER: 'POROSI SHITJE', DELIVERY_NOTE: 'FLETË DALJE', SALES_INVOICE: 'FATURË SHITJE', SALES_RETURN: 'KTHIM NGA KLIENTI',
...
export function installPhase97DocumentPdfRoutes({ app, pool, authRequired, assertCompanyAccess }) {
  app.get('/api/documents/:id/pdf', authRequired, async (req, res, next) => {
    try { const source = await businessDocument(pool, req.user, req.params.id); await assertCompanyAccess(req.user, source.company_id); const doc = pdfResponse(res, `${label(source.doc_type)}_${source.document_no}`); if (['PURCHASE_RECEIPT', 'SUPPLIER_RETURN', 'PURCHASE_RETURN', 'DELIVERY_NOTE', 'SALES_RETURN'].includes(source.doc_type)) movementPdf(doc, source); else invoicePdf(doc, source); doc.end(); } catch (error) { next(error); }
  });
