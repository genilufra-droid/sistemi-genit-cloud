  ...
  async function saveDocument(req, res, next, documentId = null) {
    ...
    try {
      ...
      for (const allocation of input.allocations) {
        ...
        const expectedInvoiceType = input.documentType.endsWith('_PAYMENT') ? 'PURCHASE_INVOICE' : 'SALES_INVOICE';
        const invoiceType = await client.query('SELECT doc_type FROM business_documents WHERE id=$1', [invoice.id]);
        if (invoiceType.rows[0]?.doc_type !== expectedInvoiceType) {
          throw requestError(input.documentType.endsWith('_PAYMENT')
            ? 'Pagesa e furnitorit lidhet vetëm me Faturë Blerje.'
            : 'Arkëtimi i klientit lidhet vetëm me Faturë Shitje.',400);
        }
        const paid = await client.query(
          `SELECT COALESCE(SUM(pa.amount),0)::numeric AS amount
           FROM payment_allocations pa
           JOIN finance_documents fd ON fd.id=pa.finance_document_id
           WHERE pa.business_document_id=$1 AND fd.status='POSTED' AND fd.id<>$2`,
          [invoice.id,id],
        );
        if (num(paid.rows[0].amount) + num(allocation.amount) > num(invoice.total_amount) + 0.0001) {
          throw requestError('Pagesa tejkalon detyrimin e mbetur të faturës.',409);
        }
        await client.query(
