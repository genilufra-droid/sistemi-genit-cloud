...
const documentImport="import { installPhase71ManufacturingDocumentRoutes } from './phase71-manufacturing-documents.js';";
...
if(!source.includes(documentImport)) source=source.replace(importLine,importLine+'\n'+documentImport);
...
if(!source.includes('installPhase71ManufacturingDocumentRoutes({')){
  const anchor="installPhase71OdooManufacturingRoutes({ app:capturedApp, pool, authRequired, requireRoles, assertCompanyAccess, accessibleCompanyIds, audit, emitTenant });";
  source=source.replace(anchor,anchor+'\ninstallPhase71ManufacturingDocumentRoutes({ app:capturedApp, pool, authRequired });');
}
source=source.replace(
  ...
  "{ group:'Gjurmueshmëri 360°',phase:6.9,active:true,items:['Ferma & Origjina','Katalogu me 165 kode BioBes','Periudha I/II/III','Bimët','Formulari i Peshës me Origjinë Opsionale','Kontroll Cilësie','Faturë Blerje','Fletë-Hyrje & Etiketë 58 mm','Lote B0/B1','Proces & Gjendje B6','Loti Final i Shitjes B2/B3','Dosja e Dokumenteve'] },\n    { group:'Prodhimi Odoo',phase:7.1,active:true,items:['Proceset','Makineritë / Qendrat e Punës','Mostrat e Klientit','Fushatat e Prodhimit','Urdhrat e Punës','Kontrolli i Cilësisë','Transferimet Proces–Proces','Paketimi','Përbërja e Lotit Final','Dokumente të Klikueshme'] },"
);
