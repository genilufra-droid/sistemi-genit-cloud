...
export async function migrateGlobalMasterData(db) {
  await db.query(`
    CREATE TABLE IF NOT EXISTS global_master_records (
      ...
      CONSTRAINT global_master_entity_check CHECK (entity_type IN ('FARMER','DRIVER','ROUTE','AGENT','ASSET','EXPENSE_CATEGORY'))
    );
...
export function installGlobalMasterDataRoutes({ app, pool, authRequired, assertCompanyAccess, accessibleCompanyIds, audit, emitTenant }) {
  ...
  app.get('/api/master-data/capabilities', authRequired, (req, res) => {
    ...
    capabilities.push({ entityType: 'CASH_ACCOUNT', permission: 'masters.manage', canCreate: ['SUPER_ADMIN','COMPANY_ADMIN','MANAGER','FINANCIER','ARKETAR'].includes(req.user.role), native: true });
    capabilities.push({ entityType: 'BANK_ACCOUNT', permission: 'masters.manage', canCreate: ['SUPER_ADMIN','COMPANY_ADMIN','MANAGER','FINANCIER','ARKETAR'].includes(req.user.role), native: true });
