...
async function createOutputLot(client, { user, companyId, warehouseId, product, sourceDate, lotType, qualityStatus, quantity, unitCost, origin, sourceDocumentId, sourceDocumentNo, sourceProcessOrderId = null, sourcePackagingOrderId = null, movementType, stockMovementType, expiryDate = null, notes = '' }) {
...
export async function migratePhase4ProcessingPackaging(db) {
  await db.query(`
    ...
    ALTER TABLE trace_lots ADD COLUMN IF NOT EXISTS source_process_order_id UUID REFERENCES process_orders(id) ON DELETE RESTRICT;
